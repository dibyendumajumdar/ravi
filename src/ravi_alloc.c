...
static RAVI_ALWAYS_INLINE uint32_t lj_fls(uint32_t x)
{
...
static RAVI_ALWAYS_INLINE uint32_t lj_ffs(uint32_t x)
{
...
#define LJ_ALLOC_MBITS		64	

#endif

#endif

#if RAVI_TARGET_LINUX
...
/* page-align a size */
#define page_align(S)\
 (((S) + (RAVI_PAGESIZE - SIZE_T_ONE)) & ~(RAVI_PAGESIZE - SIZE_T_ONE))
...
static void *direct_alloc(size_t nb)
{
  ...
  if (RAVI_LIKELY(mmsize > nb)) {     /* Check for wrap around 0 */
...
static void *alloc_sys(mstate m, size_t nb)
{
  ...
  if (RAVI_UNLIKELY(nb >= DEFAULT_MMAP_THRESHOLD)) {
...

  {
    ...
    if (RAVI_LIKELY(rsize > nb)) { /* Fail if wraps around zero */
...
void ravi_alloc_destroy(void *msp)
{
...
static void *lj_alloc_malloc(void *msp, size_t nsize)
{
  mstate ms = (mstate)msp;
  void *mem;
  size_t nb;
...
static void *lj_alloc_free(void *msp, void *ptr)
{
  if (ptr != 0) {
...
static void *lj_alloc_realloc(void *msp, void *ptr, size_t nsize)
{
