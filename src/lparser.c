...
static void ravi_typecheck(LexState *ls, expdesc *v, int *vars, TString **usertypes, int nvars,
                           int n) {
  ...
  if (n < nvars && vartype != RAVI_TANY && v->ravi_type != vartype) {
    if (v->ravi_type != vartype &&
       ...
       * better approach. The location of the OP_NEWTABLE instruction is in v->pc 
       * and we check that this has the same destination register as v->u.info which is our 
       * variable
       * local a:int[] = { 1 }
    ...
    /* if we are calling a function then convert return types */
      ...
      TString *usertype = usertypes[n];
      /* we need to make sure that a register is assigned
        so that we can emit type assertion instructions */
      luaK_exp2nextreg(ls->fs, v);
