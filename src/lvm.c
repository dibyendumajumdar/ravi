...
*/
int luaV_tointeger_(const TValue *obj, lua_Integer *p) { return luaV_tointeger(obj, p, LUA_FLOORN2I); }
...
*/
void luaV_gettable(lua_State *L, const TValue *t, TValue *key, StkId val) { GETTABLE_INLINE(L, t, key, val); }
...
** so that JIT code can invoke it
*/
void luaV_settable(lua_State *L, const TValue *t, TValue *key, StkId val) { SETTABLE_INLINE(L, t, key, val); }
...
int luaV_equalobj(lua_State *L, const TValue *t1, const TValue *t2) {
  ...
  switch (ttype(t1)) {
    ...
    case RAVI_TFCF:
      return fcfvalue(t1) == fcfvalue(t2);
    case LUA_TSHRSTR:
...
#define RKB(i) \
  check_exp(getBMode(GET_OPCODE(i)) == OpArgK, ISK(GETARG_B(i)) ? k + INDEXK(GETARG_B(i)) : base + GETARG_B(i))
#define RKC(i) \
  check_exp(getCMode(GET_OPCODE(i)) == OpArgK, ISK(GETARG_C(i)) ? k + INDEXK(GETARG_C(i)) : base + GETARG_C(i))
...
#define vmfetch                                                        \
  {                                                                    \
    ...
    op = GET_OPCODE(i);                                                \
    if (prevop != -1)                                                  \
      raviV_add_profiledata(L, prevop);                                \
    prevop = op;                                                       \
...
newframe:                       /* reentry point when frame changes (call/return) */
  ...
  for (;;) {
    ...
    vmdispatch(op) {
      ...
      vmcase(OP_RAVI_SETFIELD) vmcase(OP_RAVI_TABLE_SETFIELD) {
        TValue *rb = RKB(i);
        TValue *rc = RKC(i);
      ...
      vmcase(OP_EQ) {
        ...
        TValue *rc = RKC(i);
        int res;
        Protect(res = luaV_equalobj(L, rb, rc);) if (res != GETARG_A(i)) pc++;
        else donextjump(ci);
        vmbreak;
      }
      vmcase(OP_LT) {
        ...
        if (ttisinteger(rb) && ttisinteger(rc))
          ...
          Protect(res = luaV_lessthan(L, rb, rc);) if (res != GETARG_A(i)) pc++;
        else donextjump(ci);
        vmbreak;
      }
      vmcase(OP_LE) {
        ...
        if (ttisinteger(rb) && ttisinteger(rc))
          ...
          Protect(res = luaV_lessequal(L, rb, rc);) if (res != GETARG_A(i)) pc++;
        else donextjump(ci);
        vmbreak;
      }
      vmcase(OP_TEST) {
      ...
      vmcase(OP_TAILCALL) {
        ...
        /* See note below under OP_RETURN for why we pass the extra
           ...
           was called from OP_CALL instruction
           */
        if (luaD_precall(L, ra, LUA_MULTRET, 1 /* OP_CALL */)) { /* C function? or JIT call */
          Protect((void)0);                                      /* update 'base' */
        }
        else {
      ...
      vmcase(OP_FORPREP) {
        ...
        lua_Integer ilimit;
        int stopnow;
        if (ttisinteger(init) && ttisinteger(pstep) && luaV_forlimit(plimit, &ilimit, ivalue(pstep), &stopnow)) {
      ...
      vmcase(OP_RAVI_FORLOOP_IP) vmcase(OP_RAVI_FORLOOP_I1) {
      ...
      vmcase(OP_RAVI_FORPREP_IP) vmcase(OP_RAVI_FORPREP_I1) {
        TValue *pinit = ra;
        // TValue *plimit = ra + 1;
...
static void ravi_dump_ci(lua_State *L, CallInfo *ci) {
  ...
  for (; stack_ptr >= base; stack_ptr--, i--) {
    printf("stack[%d] reg[%d] = %s %s", i, (int)(stack_ptr - base), (stack_ptr == base ? "(base) " : ""),
           (stack_ptr == L->top ? "(L->top) " : ""));
    if (ttisCclosure(stack_ptr))
      printf("C closure\n");
    else if (ttislcf(stack_ptr))
      printf("light C function\n");
...
void ravi_debug_trace(lua_State *L, int opCode, int pc) {
  RAVI_DEBUG_STACK(char buf[100]; CallInfo *ci = L->ci; int funcpos = (int)(ci->func - L->stack);
                   int top = (int)(L->top - L->stack); int ci_top = (int)(ci->top - L->stack);
                   ...
                   printf("Stack dump %s (%s) function %d, pc=%d, L->top = %d, ci->top = %d\n", luaP_opnames[opCode],
                          buf, funcpos, pc, (top - base), (ci_top - base));
...
** so that JIT code can invoke it
*/
void raviV_gettable_i(lua_State *L, const TValue *t, TValue *key, StkId val) { GETTABLE_INLINE_I(L, t, key, val); }
...
** so that JIT code can invoke it
*/
void raviV_settable_i(lua_State *L, const TValue *t, TValue *key, StkId val) { SETTABLE_INLINE_I(L, t, key, val); }
